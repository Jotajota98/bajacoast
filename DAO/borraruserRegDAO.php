...
$borrar_administrador = mysqli_query( $con,"delete from tb_registroInfo where id='".$id."' ");
//$resultado = mysqli_num_rows($borrar_administrador);		

if($borrar_administrador ){
    header("Location: ../listado_reg.php");

}else{
    echo "<script>alert('Error al eliminar el objeto.')</script>";
    echo "<script>window.location = '../listado_reg.php'</script>";
}
